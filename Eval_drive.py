...
data_location = './'
testing_images_loc = os.path.join(data_location, 'DRIVE/test/images/')
testing_label_loc = os.path.join(data_location, 'DRIVE/test/labels/')
fov_masks_loc = os.path.join(data_location, "DRIVE/test/masks/")
output_dir = "DRIVE/test/results/"
n_test_images = 20
image_width, image_height = (584, 565)
desired_size = 592
threshold = 0.5
use_fov = True
# =================================================================== #
...
print("> Predicting on DRIVE dataset.")

x_test, y_test = load_files(testing_images_loc, testing_label_loc, desired_size, get_label_name_drive, mode='test')
...
all_masks_data = None
if use_fov:
    all_masks_data = np.ravel(load_mask_files(fov_masks_loc, testing_images_loc, get_mask_name_drive))
