...
# =================================================================== #
model_name = "Model/DROPS/SA_UNet.h5"
data_location = './'
testing_images_loc = os.path.join(data_location, 'DROPS/test/images/')
testing_label_loc = os.path.join(data_location, 'DROPS/test/labels/')
fov_masks_loc = os.path.join(data_location, "DROPS/test/masks")
output_dir = "DROPS/test/results/"
n_test_images = 4
...
print("> Predicting on DROPS dataset.")

x_test, y_test = load_files(testing_images_loc, testing_label_loc, desired_size, get_label_name_drops, mode='test')
...
all_masks_data = None
if use_fov:
    all_masks_data = np.ravel(load_mask_files(fov_masks_loc, testing_images_loc, get_mask_name_drops))
